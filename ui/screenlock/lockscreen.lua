...
-- Colors wibox
lock_screen_box.bg = beautiful.bg_false
lock_screen_box.fg = beautiful.fg_0
-- Multi Screen
awful.screen.connect_for_each_screen(function(s)
	if s == screen.primary then
		...
		s.mylockscreen = helpers.ui.screen_mask(
			s,
			beautiful.lock_screen_bg or beautiful.bg_false
		)
	end
end)
...
local function create_text_widget(index, w)
	local text_widget = wibox.widget({
		id = "t" .. index,
		markup = w,
		font = beautiful.font_mod .. "Bold 24",
		align = "center",
		valign = "center",
		forced_width = dpi(36),
...
for i, m in pairs(time_char) do
	local text = shorten.text(m, "#ffffff" .. "10")
...
local function activate_word(w)
	for i, m in pairs(char_map[w]) do
		local text = m.text
		m.markup = shorten.text(text, beautiful.white)
	end
end
...
local function deactivate_word(w)
	for i, m in pairs(char_map[w]) do
		local text = m.text
		m.markup = shorten.text(text, "#ffffff" .. "10")
	end
end
